...
class Inventory(models.Model):
    ...
    supplier_contact = models.CharField(max_length=255, blank=True, null=True)  # Supplier contact info')
    category = models.CharField(max_length=100, blank=True, null=True)  # Category for inventory items
    def __str__(self):
        return self.name

    # FIX: Explicitly cast self.quantity before operation
    # def reduce_quantity(self, quantity_to_reduce):
    #     """Reduces the quantity, ensuring it doesn't go below zero."""
    #     quantity_to_reduce = decimal.Decimal(str(quantity_to_reduce))
    #     if quantity_to_reduce <= 0:
    #         return
    #     # Cast self.quantity just in case it's a float in memory
    #     current_quantity = decimal.Decimal(str(self.quantity))
    #     if current_quantity >= quantity_to_reduce:
    #         self.quantity = current_quantity - quantity_to_reduce
    #         self.save(update_fields=['quantity'])
    #     else:
    #         raise ValidationError(f"Insufficient stock for {self.name}. Available: {current_quantity}, Required: {quantity_to_reduce}")
    ...
    def increase_quantity(self, quantity_to_increase):
        ...
        # Cast self.quantity just in case it's a float in memory
        current_quantity = decimal.Decimal(str(self.quantity))
        self.quantity = current_quantity + quantity_to_increase
        self.save(update_fields=['quantity'])
